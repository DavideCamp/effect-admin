...
pnpm --filter @your-org/web add @effect-admin/react
```

The runtime peers are the same technologies your app already uses: `effect`,
`@effect/platform`, `react`, and `react-dom`.
...
Until the packages are published, consume them through workspace dependencies
inside this repository or through the tarballs generated by `pnpm pack:all`.
